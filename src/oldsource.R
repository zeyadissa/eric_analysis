...
invest_growth <- mean((baseline %>%
                       filter(!date %in% c('2004','2005','2019')))$invest_growth)
...
GetPolicy(backlog=13000,investment=800,growth=growth,t=10,policy=0)
...
# Baseline growth
ggplot()+
  geom_line(data=FINAL_data %>% filter(trust_code == 'RGR'),aes(x=date,y=total_growth)) +
  geom_line(data=baseline%>% filter(date >2005),aes(x=date,y=total_growth,group=1),lwd=1,col='red') +
  ylim(-1,1) +
  geom_hline(yintercept=0,linetype=2,lwd=1.5,col='black')+
  theme_bw()
...
ggplot()+
  ...
  geom_function(fun=function(x){GetPolicy(backlog=11637,
                                          investment=615,
                                          growth=min_underlying_growth,
                                          t=10,
                                          policy=x)},col=thf2) +
  geom_function(fun=function(x){GetPolicy(backlog=11637,
                                          investment=615,
                                          growth=max_underlying_growth,
                                          t=10,
                                          policy=x)},col=thf3) +
...
ggplot()+
  geom_function(fun=function(x){GetBacklog(backlog=13000,
                                          investment=800,
